...
function ENT:AddStuff()
	...
	e1:SetModel("models/sentry/tiger_gun1.mdl")
	e1:SetPos(self:LocalToWorld(Vector(177,0,27)))
	e1:SetAngles(ang)
	e1:SetSkin(Skin)
	e1:Spawn()
	e1:Activate()
	...
	constraint.AdvBallsocket(e1,self,0,0,Vector(0,0,0),self:WorldToLocal(e1:LocalToWorld(Vector(0,0,1))),0,0,0,0,ang.y-90,0,0,ang.y+90,0,0,0,0,1)
	e1:SetNotSolid(true)
	self:AddOnRemove(e1)
	self.GunMount1=e1
	e1.wac_ignore=true
	...
	e2:SetModel("models/sentry/tiger_gun2.mdl")
	e2:SetPos(self:LocalToWorld(Vector(177,0,27)))
	e2:SetAngles(ang)
	e2:SetSkin(Skin)
	e2:Spawn()
	e2:Activate()
	...
	constraint.AdvBallsocket(e2,e1,0,0,Vector(2,0,0),e1:WorldToLocal(e2:LocalToWorld(Vector(2,0,1))),0,0,0,ang.p-30,0,0,ang.p+15,0,0,0,0,0,1)
	e2:SetNotSolid(true)
	self:AddOnRemove(e2)
	self.GunMount2=e2
	e2.wac_ignore=true
	...
	e3:SetPos(e2:LocalToWorld(Vector(-5,0,0)))
	e3:SetAngles(ang)
	e3:SetSkin(Skin)
	...
	constraint.AdvBallsocket(e4,self,0,0,Vector(0,0,0),self:WorldToLocal(e4:LocalToWorld(Vector(0,0,1))),0,0,0,0,ang.y-90,0,0,ang.y+90,0,0,0,0,1)
	e4:SetNotSolid(true)
	self.Radar1=e4
	self:AddOnRemove(e4)
	e4.wac_ignore=true
	...
	constraint.AdvBallsocket(e5,e4,0,0,Vector(0,0,0),e4:WorldToLocal(e5:LocalToWorld(Vector(0,0,1))),0,0,0,ang.p-30,0,0,ang.p+15,0,0,0,0,0,1)
	e5:SetNotSolid(true)
	self:AddOnRemove(e5)
	self.Radar2=e5
